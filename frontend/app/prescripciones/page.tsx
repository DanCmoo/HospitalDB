        ...
        <table className="min-w-full">
          <thead className="bg-gray-50">
            <tr>
              ...
              <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Cita</th>
              <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Paciente</th>
              <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Dosis</th>
              <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Frecuencia (hrs)</th>
          ...
          <tbody className="divide-y divide-gray-200">
            {filteredPrescripciones.map((prescripcion) => (
              <tr key={`${prescripcion.codMed}-${prescripcion.idCita}`} className="hover:bg-gray-50">
                <td className="px-6 py-4 text-sm">
                  {prescripcion.medicamento?.nomMed || 'N/A'}
                  <div className="text-xs text-gray-500">Stock: {prescripcion.medicamento?.stock || 0}</div>
                </td>
                <td className="px-6 py-4 whitespace-nowrap text-sm">#{prescripcion.idCita}</td>
                <td className="px-6 py-4 text-sm">{prescripcion.cita?.paciente?.persona?.nomPers || 'N/A'}</td>
                <td className="px-6 py-4 whitespace-nowrap text-sm">{prescripcion.dosis}</td>
                <td className="px-6 py-4 whitespace-nowrap text-sm">{prescripcion.frecuencia}</td>
                <td className="px-6 py-4 whitespace-nowrap text-sm">{prescripcion.duracion}</td>
                <td className="px-6 py-4 whitespace-nowrap text-sm">{prescripcion.fechaEmision}</td>
          ...
          <div className="bg-white rounded-lg p-6 max-w-2xl w-full max-h-[90vh] overflow-y-auto">
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  <label className="block text-sm font-medium mb-1">Frecuencia (horas)</label>
                  <input
                    type="number"
                    min="1"
